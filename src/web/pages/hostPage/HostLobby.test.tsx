...
const MockAudio = vi.fn(function() { this.load = mockLoad; });
...
describe("HostLobby", () => {
    ...
    it("renders the game code text", () => {
        const { container } = render(
            <HostLobby teams={makeTeams()} gameCode="ABC123" onStartGame={vi.fn()} />
        );
    ...
    it("renders the player URI link with correct href containing the gameCode", () => {
        const { container } = render(
            <HostLobby teams={makeTeams()} gameCode="XYZ789" onStartGame={vi.fn()} />
        );
    ...
    it("clicking Start Game calls onStartGame callback", () => {
        ...
        const { container } = render(
            <HostLobby teams={makeTeams()} gameCode="CODE" onStartGame={onStartGame} />
        );
        const button = container.querySelector("button")!;
